...
add_action('wp_ajax_projmanpro_project_quick_view', function() {
    ...
    // Meta fields
    $status_terms = wp_get_post_terms($post_id, 'projmanpro_project_status');
    $status       = $status_terms[0]->slug ? $status_terms[0]->slug : '—';

    $priority_terms = wp_get_post_terms($post_id, 'projmanpro_project_priority');
...
add_action('wp_ajax_projmanpro_project_add_comment', function() {
    ...
    $post_id = isset($_POST['post_id']) ? intval($_POST['post_id']) : 0;
    if (!$post_id) wp_send_json_error(['message' => 'Invalid project ID']);
